...
| 本地数据导出 JSON 备份 | MVP | ✅ |
| PWA 离线 + 新版本提示 | MVP | ✅ |
| 坚果云双人协同同步（多端一致、冲突处理） | Sync | 🚧 开发中 |
| 统计（时间窗/成员/类型筛选、月度趋势） | Sync | 🚧 |
...
| 坚果云账号 | 观影数据云端同步（Sync 阶段使用，可先不办） | [jianguayun.com](https://www.jianguayun.com) 注册 |
...
**Q：我的数据存在哪？安全吗？**
观影记录存在你手机本地 IndexedDB；Sync 阶段上线后同步到你自己的坚果云。TMDB Key 存在 Cloudflare 环境变量，不进前端代码；Worker 不存储、不写日志。在当前阶段（MVP，云同步未上线），请养成**每月导出一次 JSON 备份**的习惯（设置 → 数据）。
...
**Q：换了手机 / 清了浏览器数据怎么办？**
当前 MVP 阶段：导入之前导出的 JSON 备份（导入功能随 Sync 阶段提供，目前可先保留旧手机导出备份）。Sync 阶段上线后：新手机登录同一坚果云账号即可全量拉回。

**Q：两台手机怎么保持数据一致？**
这正是 Sync 阶段（开发中）的能力：双方在设置页填各自的坚果云账号+应用密码，数据以"一事件一文件"形式同步到同一个坚果云目录，自动对账、冲突提示。
...
│  └─ src/
│     ├─ pages/            # 路由页面（首页/录入/详情/设置）
│     ├─ components/       # 规范组件（RecordCard/MoviePicker/...）
│     ├─ services/         # TMDB / 海报缓存 / Worker 基址 / 备份
│     ├─ db/               # Dexie（IndexedDB）schema 与读写
...
- 坚果云凭据（Sync 阶段）只存各自手机本地，绝不上传、不进代码
